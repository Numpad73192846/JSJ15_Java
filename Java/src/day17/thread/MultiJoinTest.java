...
public class MultiJoinTest extends Thread {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		t1.start();
		t2.start();
		t1.join();		// 메인 스레드를 t1 스레드 종료까지 일시 정지
		t2.join();		// 메인 스레드를 t2 스레드 종료까지 일시 정지
						// 다시 메인 스레드 시작
		...
		s3.start();
		s4.start();
		s1.join();		// 메인 스레드를 s1 스레드 종료까지 일시 정지
		s2.join();		// 메인 스레드를 s2 스레드 종료까지 일시 정지
		s3.join();		// 메인 스레드를 s3 스레드 종료까지 일시 정지
		s4.join();		// 메인 스레드를 s4 스레드 종료까지 일시 정지
						// 다시 메인 스레드 시작
